...
#include "../include/tensor_cpp/tensor.h"
#include "../include/tensor_cpp/ops.h"
#include <iostream>
#include <chrono>
#include <iomanip>
#include <cassert>
...
bool allclose(const Tensor& a, const Tensor& b, float rtol = 1e-5f, float atol = 1e-8f) {
    ...
    for (size_t i = 0; i < a.size(); ++i) {
        ...
        float tolerance = atol + rtol * std::abs(b[i]);
        if (diff > tolerance) {
...
void test_embedding() {
    ...
    try {
        ...
        for (size_t i = 0; i < indices_data.size(); ++i) {
            indices_data[i] = static_cast<long>(i % 100);  // Some indices
        }
        TensorL indices(indices_data, Shape({2, 10}));  // Batch of 2, sequence length 10
