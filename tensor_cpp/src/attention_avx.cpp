...
Tensor self_attention_avx2(
    ...
    // Key/value may have different sequence length (e.g., with KV cache)
    const Shape& k_shape = k.shape();
    size_t k_seq_len = k_shape[2];

    // Output: [batch, num_heads, q_seq_len, head_dim]
    size_t output_size = batch * num_heads * q_seq_len * head_dim;
    std::vector<float> output_data(output_size);

    #pragma omp parallel for if(batch * num_heads * q_seq_len > 10)
    for (size_t b = 0; b < batch; ++b) {
        for (size_t h = 0; h < num_heads; ++h) {
            // Process this head
            size_t head_offset = (b * num_heads + h) * q_seq_len * head_dim;
            ...
            for (size_t i = 0; i < q_seq_len; ++i) {
                ...
                for (size_t j = 0; j < k_seq_len; ++j) {
                    ...
                    size_t k_offset = ((b * num_heads + h) * k_seq_len + j) * head_dim;
                ...
                if (causal_mask != nullptr) {
                    ...
                    const Shape& mask_shape = causal_mask->shape();

                    // Handle different mask shapes:
                    // [1, 1, 1, k_seq_len] -> use j directly
                    // [1, 1, q_seq_len, k_seq_len] -> use i * k_seq_len + j
                    for (size_t j = 0; j < k_seq_len; ++j) {
                        size_t mask_idx;
                        if (mask_shape[2] == 1) {
                            // Mask is [1, 1, 1, k_seq_len]
                            mask_idx = j;
                        } else {
                            // Mask is [1, 1, q_seq_len, k_seq_len]
                            mask_idx = i * k_seq_len + j;
                        }
                ...
                // Compute exp and sum
                std::vector<float> exp_scores(k_seq_len);
                float sum_exp = 0.0f;
                for (size_t j = 0; j < k_seq_len; ++j) {
                ...
                for (size_t d = 0; d < head_dim; ++d) {
                    ...
                    for (size_t j = 0; j < k_seq_len; ++j) {
                        if (exp_scores[j] > 0.0f) {
                            ...
                            size_t v_offset = ((b * num_heads + h) * k_seq_len + j) * head_dim;
