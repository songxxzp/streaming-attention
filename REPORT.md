...
本文在8×2×26核心（416总核心）的CPU集群上进行了系统实验。结果表明：（1）Streaming attention相比传统GEMM并行方法实现了2.61倍（单机）至6.41倍（多机）的性能提升；（2）OpenMP单机扩展性在16线程时达到峰值，受限于单节点内存带宽饱和；（3）MPI多机并行通过增加聚合内存带宽实现了接近线性的扩展性。实验结果验证了：对于CPU上的memory-bound Attention工作负载，算法级并行设计和聚合带宽扩展比底层算子优化更为关键。
...
```
单机内存带宽: B_single ≈ 100 GB/s (实测)
理论峰值: B_peak ≈ 120 GB/s
...
| ----------------------- | -------------------------------------- | --------------------------- |
| **服务器节点数**  | 8台                                    | 分布式集群                  |
| **每节点CPU**     | 2 × Intel Xeon Cascadelake (2-socket) | 每个socket 26核心           |
...
> The performance advantage of streaming attention stems from algorithmic design rather than low-level kernel tuning. 通过避免materialize中间矩阵，streaming attention显著减少了内存访问和同步开销，这是CPU平台上memory-bound工作负载的优化关键。
...
> Although OpenMP provides fine-grained parallelism, MPI-based multi-node parallelism achieves higher scalability due to increased memory bandwidth rather than reduced computation time. 对于memory-bound的Attention计算，**增加聚合内存带宽比增加计算并行度更有效**。

**通信开销分析**：

```
MPI通信量: T × d × sizeof(float) = 65536 × 128 × 4 = 32 MB

16节点通信时间: T_comm ≈ 0.1-0.2 ms (实测)
16节点计算时间: T_comp ≈ 1.3 ms
通信占比: 0.15 / (1.3 + 0.15) ≈ 10%
...
| T     | 1节点  | 2节点  | 4节点  | 8节点  | 16节点 | 效率@16节点 |
| ----- | ------ | ------ | ------ | ------ | ------ | ----------- |
...
| 应用场景   | 序列长度 | 推荐方法                        | 预期性能                 |
| ---------- | -------- | ------------------------------- | ------------------------ |
| 实时对话   | 2K-4K    | Streaming+OMP (8-16线程)        | 15-19M tokens/s          |
| 文档摘要   | 8K-16K   | Streaming+MPI (2-4节点)         | 25-30M tokens/s          |
| 长文本分析 | 32K-64K  | Streaming+MPI+OMP (16MPI×4OMP) | **46.7M tokens/s** |
...
./streaming_omp --seq_len 8192 --block_size 128

# 多机推理（最优：16MPI×4OMP）
mpirun -np 16 --hostfile hosts \
  --map-by ppr:4:node --bind-to core \
  ./streaming_mpi --seq_len 65536 --block_size 128
```
